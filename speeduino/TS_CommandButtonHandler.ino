...
void TS_CommandButtonsHandler(uint16_t buttonCommand) {
  switch (buttonCommand) {
    ...
    case TS_CMD_INJ1_50PC:  // cmd group is for injector1 50% dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_INJ_50pc, INJ1_CMD_BIT);
      }
      if (!BIT_CHECK(HWTest_INJ_50pc, INJ1_CMD_BIT)) {
        closeInjector1();
    ...
    case TS_CMD_INJ2_50PC:  // cmd group is for injector2 50%dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_INJ_50pc, INJ2_CMD_BIT);
      }
      if (!BIT_CHECK(HWTest_INJ_50pc, INJ2_CMD_BIT)) {
        closeInjector2();
      }  // Ensure this output is turned off (Otherwise the output may stay on permanently)
      break;
    ...
    case TS_CMD_INJ3_50PC:  // cmd group is for injector3 50%dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_INJ_50pc, INJ3_CMD_BIT);
      }
    ...
    case TS_CMD_INJ4_50PC:  // cmd group is for injector4 50% dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_INJ_50pc, INJ4_CMD_BIT);
      }
      if (!BIT_CHECK(HWTest_INJ_50pc, INJ4_CMD_BIT)) {
        closeInjector4();
      }  // Ensure this output is turned off (Otherwise the output may stay on permanently)
      break;
    ...
    case TS_CMD_INJ5_50PC:  // cmd group is for injector5 50%dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_INJ_50pc, INJ5_CMD_BIT);
      }
      if (!BIT_CHECK(HWTest_INJ_50pc, INJ5_CMD_BIT)) {
        closeInjector5();
      }  // Ensure this output is turned off (Otherwise the output may stay on permanently)
      break;
    ...
    case TS_CMD_INJ6_50PC:  // cmd group is for injector6 50% dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_INJ_50pc, INJ6_CMD_BIT);
      }
      if (!BIT_CHECK(HWTest_INJ_50pc, INJ6_CMD_BIT)) {
        closeInjector6();
      }  // Ensure this output is turned off (Otherwise the output may stay on permanently)
      break;
    ...
    case TS_CMD_INJ7_50PC:  // cmd group is for injector7 50%dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_INJ_50pc, INJ7_CMD_BIT);
      }
      if (!BIT_CHECK(HWTest_INJ_50pc, INJ7_CMD_BIT)) {
        closeInjector7();
      }  // Ensure this output is turned off (Otherwise the output may stay on permanently)
      break;
    ...
    case TS_CMD_INJ8_50PC:  // cmd group is for injector8 50% dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_INJ_50pc, INJ8_CMD_BIT);
      }
      if (!BIT_CHECK(HWTest_INJ_50pc, INJ8_CMD_BIT)) {
        closeInjector8();
      }  // Ensure this output is turned off (Otherwise the output may stay on permanently)
      break;
    ...
    case TS_CMD_IGN1_50PC:  // cmd group is for spark1 50%dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_IGN_50pc, IGN1_CMD_BIT);
      }
      if (!BIT_CHECK(HWTest_IGN_50pc, IGN1_CMD_BIT)) {
        endCoil1Charge();
      }  // Ensure this output is turned off (Otherwise the output may stay on permanently)
      break;
    ...
    case TS_CMD_IGN2_50PC:  // cmd group is for spark2 50%dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_IGN_50pc, IGN2_CMD_BIT);
      }
      if (!BIT_CHECK(HWTest_IGN_50pc, IGN2_CMD_BIT)) {
        endCoil2Charge();
      }  // Ensure this output is turned off (Otherwise the output may stay on permanently)
      break;
    ...
    case TS_CMD_IGN3_50PC:  // cmd group is for spark3 50%dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_IGN_50pc, IGN3_CMD_BIT);
      }
    ...
    case TS_CMD_IGN4_50PC:  // cmd group is for spark4 50%dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_IGN_50pc, IGN4_CMD_BIT);
      }
      if (!BIT_CHECK(HWTest_IGN_50pc, IGN4_CMD_BIT)) {
        endCoil4Charge();
      }  // Ensure this output is turned off (Otherwise the output may stay on permanently)
      break;
    ...
    case TS_CMD_IGN5_50PC:  // cmd group is for spark4 50%dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_IGN_50pc, IGN5_CMD_BIT);
      }
      if (!BIT_CHECK(HWTest_IGN_50pc, IGN5_CMD_BIT)) {
        endCoil5Charge();
      }  // Ensure this output is turned off (Otherwise the output may stay on permanently)
      break;
    ...
    case TS_CMD_IGN6_50PC:  // cmd group is for spark6 50%dc actions
      if (BIT_CHECK(currentStatus.testOutputs, 1)) {
        BIT_TOGGLE(HWTest_IGN_50pc, IGN6_CMD_BIT);
      }
      if (!BIT_CHECK(HWTest_IGN_50pc, IGN6_CMD_BIT)) {
        endCoil6Charge();
      }  // Ensure this output is turned off (Otherwise the output may stay on permanently)
      break;
    ...
    //   if (BIT_CHECK(currentStatus.testOutputs, 1)) {
    //     BIT_TOGGLE(HWTest_IGN_50pc, IGN7_CMD_BIT);
    //   }
    //   if (!BIT_CHECK(HWTest_IGN_50pc, IGN7_CMD_BIT)) {
    //     endCoil7Charge();
    //   }  // Ensure this output is turned off (Otherwise the output may stay on permanently)
    //   break;
    ...
    //   if (BIT_CHECK(currentStatus.testOutputs, 1)) {
    //     BIT_TOGGLE(HWTest_IGN_50pc, IGN8_CMD_BIT);
    //   }
    //   if (!BIT_CHECK(HWTest_IGN_50pc, IGN8_CMD_BIT)) {
    //     endCoil8Charge();
    //   }  // Ensure this output is turned off (Otherwise the output may stay on permanently)
    //   break;
