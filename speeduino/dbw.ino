...
void dbw() {
  if (configPage10.dbwEnabled == 1) {
    ...
    // Idle or Driving maps
    if (currentStatus.pedal < (1 * 2) && currentStatus.MAP < 70) {
      DBW_TARGET = get3DTableValue(&dbwIdleTable, (currentStatus.pedal * 2), currentStatus.RPM); // Idle
    } else {
      DBW_TARGET = get3DTableValue(&dbwTable, currentStatus.MAP, currentStatus.RPM); // Driving
    }
